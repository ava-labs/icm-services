...
var (
	...
	messageProtocolConfig  = config.MessageProtocolConfig{
		MessageFormat: config.TELEPORTER_V2.String(),
...
func init() {
	...
	validTeleporterMessage = teleportermessengerv2.TeleporterMessageV2{
		...
		OriginTeleporterAddress: common.HexToAddress("0x0123456789abcdef0123456789abcdef01234567"),
...
func TestShouldSendMessage(t *testing.T) {
	...
	messageReceivedInput, err := teleportermessengerv2.PackMessageReceived(messageID)
	require.NoError(t, err)

	messageNotDelivered, err := teleportermessengerv2.PackMessageReceivedOutput(false)
	require.NoError(t, err)

	messageDelivered, err := teleportermessengerv2.PackMessageReceivedOutput(true)
	require.NoError(t, err)
...
func TestSendMessageAlreadyDelivered(t *testing.T) {
	...
	messageReceivedCallData, err := teleportermessengerv2.PackMessageReceived(messageID)
	require.NoError(t, err)
	...
	messageDeliveredResult, err := teleportermessengerv2.PackMessageReceivedOutput(true)
	require.NoError(t, err)
