...
func (f *factory) NewMessageHandler(
	...
	teleporterMessage, err := parseTeleporterMessageV2(unsignedMessage)
	if err != nil {
		logger.Error(
...
func (m *messageHandler) ShouldSendMessage() (bool, error) {
	...
	teleporterMessenger := m.getTeleporterMessengerV2()
...
func (m *messageHandler) SendMessage(signedMessage *warp.Message) (common.Hash, error) {
	...
	teleporterMessage, err := parseTeleporterMessageV2(&signedMessage.UnsignedMessage)
	if err != nil {
		m.logger.Error("Failed to parse teleporter message V2", zap.Error(err))
		return common.Hash{}, err
	}
	...
	if receipt.Status != types.ReceiptStatusSuccessful {
		// Check if the message has already been delivered to the destination chain
		delivered, err := m.getTeleporterMessengerV2().MessageReceived(&bind.CallOpts{}, m.teleporterMessageID)
		if err != nil {
			log.Error(
				"Failed to check if message has been delivered to destination chain.",
				zap.Error(err),
...
func (m *messageHandler) getTeleporterMessengerV2() *teleportermessengerv2.TeleporterMessengerV2 {
