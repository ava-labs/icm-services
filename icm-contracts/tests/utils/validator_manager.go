...
import (
	...
	acp99manager "github.com/ava-labs/icm-services/abi-bindings/go/validator-manager/ACP99Manager"
	erc20tokenstakingmanager "github.com/ava-labs/icm-services/abi-bindings/go/validator-manager/ERC20TokenStakingManager"
	examplerewardcalculator "github.com/ava-labs/icm-services/abi-bindings/go/validator-manager/ExampleRewardCalculator"
	...
	for {
		...
		case <-ctx.Done():
			Expect(ctx.Err()).Should(BeNil(),
				"node %s did not report positive uptime for validation %s within %s (last error: %v, last seen: %+v)",
				vdr.NodeID,
				validationID,
				validatorUptimeWaitTimeout,
				err,
				reported,
			)
		}
