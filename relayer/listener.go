...
func (lstnr *Listener) processLogs(ctx context.Context) error {
	...
	for {
		...
		case err := <-lstnr.errChan:
			...
			lstnr.logger.Error("Received error", zap.Error(err))
		case icmBlockInfo := <-lstnr.Subscriber.ICMBlocks():
			// If this is the first subscriber block either or startup or after a reconnect.
			if catchingUp {
			...
			// Catchup blocks will have a blocknumber less than or equal to the last processed subscriber block.
			if icmBlockInfo.BlockNumber > lstnr.lastSubscriberBlockProcessed {
