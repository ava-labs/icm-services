...
AVALANCHE_SOLIDITY_VERSION=$(FOUNDRY_PROFILE=avalanche forge config | awk -F"=" '/^solc/ {print $2}' | tr -d \")
AVALANCHE_EVM_VERSION=$(FOUNDRY_PROFILE=avalanche forge config | awk -F"=" '/^evm_version/ {print $2}' | tr -d \")
